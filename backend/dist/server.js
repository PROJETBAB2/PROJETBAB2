...
function isAdminJwtPayload(value) {
...
function requireAdmin(req, res, next) {
    ...
    const token = m === null || m === void 0 ? void 0 : m[1];
    if (!token)
...
app.post("/api/admin/setup", async (req, res) => {
    try {
        ...
        const passwordHash = await bcryptjs_1.default.hash(p, 10);
        ...
        return res.status(500).json({ error: (e === null || e === void 0 ? void 0 : e.message) || "Erreur serveur" });
    }
});
app.post("/api/admin/login", async (req, res) => {
    try {
        ...
        const ok = await bcryptjs_1.default.compare(p, user.passwordHash);
        if (!ok)
            return res.status(401).json({ error: "Identifiants invalides" });
        const token = signAdminToken({ id: user.id, email: user.email });
        return res.json({
...
app.get("/api/admin/me", requireAdmin, async (req, res) => {
    try {
        const id = Number(req.adminUserId);
...
function dishImageUrl(filename) {
    if (!filename)
        return "";
    if (filename.startsWith("http") || filename.startsWith("/"))
        return filename;
    return `/uploads/${path_1.default.basename(filename)}`;
}
...
function shouldBlockTableAtEvalTime(reservation, evalMinutes) {
    ...
    try {
        ...
        const table = await prisma.table.create({
            data: {
                name,
                capacity,
                posX: posX !== null && posX !== void 0 ? posX : 50,
                posY: posY !== null && posY !== void 0 ? posY : 50,
    ...
    try {
        ...
        const availableTables = tables.filter((table) => {
            ...
            return !resForTable.some((r) => {
                ...
                const rEnd = eh * 60 + em;
                if (!shouldBlockTableAtEvalTime(r, startMinutes))
                    return false;
...
app.post("/api/dishes", requireAdmin, async (req, res) => {
    try {
        ...
        if (!name)
            return res.status(400).json({ error: "Nom du plat requis" });
        ...
        return res.status(500).json({ error: (e === null || e === void 0 ? void 0 : e.message) || "Erreur création du plat" });
    }
});
    ...
    try {
        ...
        if (imageBase64 && typeof imageBase64 === "string") {
            if (existing.imageUrl) {
                const oldPath = path_1.default.join(UPLOAD_DIR, path_1.default.basename(existing.imageUrl));
        ...
        return res.status(500).json({ error: (e === null || e === void 0 ? void 0 : e.message) || "Erreur modification du plat" });
    }
});
app.delete("/api/dishes/:id", requireAdmin, async (req, res) => {
    try {
        ...
        if (!existing)
            return res.status(404).json({ error: "Plat introuvable" });
        if (existing.imageUrl) {
            const p = path_1.default.join(UPLOAD_DIR, path_1.default.basename(existing.imageUrl));
